...
def go_to_joint_state():
    ...
    msg = String()
    msg.data = "go_to_joint_state," + ','.join(map(str, joint_state_values))
    ui_command_pub.publish(msg)

# Function to plan cartesian path
def plan_cartesian_path():
    cartesian_path_values = [slider.get() for slider in cartesian_path_sliders]
    msg = String()
    msg.data = "plan_cartesian_path," + ','.join(map(str, cartesian_path_values))
    ui_command_pub.publish(msg)
...
# ROS initialization
rclpy.init()
node = rclpy.create_node('arctos_control_gui')
ui_command_pub = node.create_publisher(String, '/ui_command', 10)
joint_states_sub = node.create_subscription(JointState, '/joint_states', update_joint_sliders, 10)
transformed_tf_sub = node.create_subscription(TransformStamped, '/transformed_tf', update_cartesian_sliders, 10)
